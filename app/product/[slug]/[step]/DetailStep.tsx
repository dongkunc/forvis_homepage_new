...
> = {
  align: {
    p1: { title: "얼라인 비전", bullets: ["부품 조립/전극/필름 부착 위치 검증 최적화된 비전", "미세 부품조립부터 롤투롤 생산 보정까지", "이 모든 것을 구현해 드립니다."] },
    p2: { title: "정렬·검증 알고리즘", subs: ["제일 중요한 커스텀 프로그래밍", "디자인부터 기능까지", "모든 항목을 사용자 우선으로."] },
    tabs: [
      { key: "camera", title: "카메라", body: ["정지상태에서. 한 장의 이미지로 간단하지만 정교한 위치 확인", "이동중에. 길이 제한이 없는 고해상도 이미지로 정교한 위치 확인", "1MP~50MP Area 카메라", "2K ~ 8K Line 카메라"] },
  ...
  cis: {
    p1: { title: "롤투롤 전면 검사", bullets: ["CIS 카메라 기반 전면 스캔", "왜곡없는 이미지, 폭 측정·결함 검출 최적화", "설치공간 최소화로 공간 활용성 최대."] },
    p2: { title: "대면적 인라인 스캐닝", subs: ["제일 중요한 커스텀 프로그래밍", "디자인부터 기능까지", "모든 항목을 사용자 우선으로."] },
    tabs: [
      { key: "camera", title: "카메라", body: ["검사할 대상체에 따른 카메라 결정", "이동중에. 길이 제한이 없는 고해상도 이미지로 정교한 위치 확인", "600Dpi ~ 900Dpi. Line 카메라", "300mm ~ 1500mm. Line 카메라"] },
      { key: "light", title: "조명", body: ["불량의 시인성에 따라 선택", "반사조명. 빛의 반사를 통하여 불량검출", "투과조명. 빛의 투과량을 통하여 불량검출", "그외. 제품에 맞는 맞춤 조명 솔루션"] },
      { key: "pc", title: "PC/소프트웨어", body: ["SW. 레시피 관리, 통계/리포트, 트레이스 로그", "PC. 검사속도에 반영하여", "일반검사. CPU i5, RAM 16Gb, SSD 500Gb, Window 10", "고속/딥러닝 검사. 일반검사에 GPU 추가, CPU i9"] },
      { key: "method", title: "검사방식", body: ["이미지 필터링(Filter, Blurring, Sharpening)", "서브픽셀 에지/코너", "ROI(Region of Interest) 설정 및 추출", "이진화(Binarization) ", "형태학적 연산(Morphological Operations)", "객체 추적(Object Tracking)", "영상 정합(Image Registration) "] },
      { key: "speed", title: "검사속도", body: ["원하는 속도에 맞춰", "30kHz ~ 60kHz"] },
      { key: "result", title: "불량크기", body: ["작은 불량까지", "um크기의 작은 불량에서 mm크기의 불량까지 검출 가능"] },
    ],
  },
  sheet: {
    p1: { title: "시트 단위 검사", bullets: ["시트 단위 검사에 최적화", "공간, 검사속도, 검사제품, 찾아내고 싶은 불량의 크기", "에지 서브픽셀 추출로 μm 단위 급 반복 정밀도", "이 모든 것을 구현해 드립니다."] },
    p2: { title: "내가 원하는 것은.", subs: ["제일 중요한 커스텀 프로그래밍", "디자인부터 기능까지", "모든 항목을 사용자 우선으로."] },
    tabs: [
      { key: "camera", title: "카메라", body: ["정지상태에서. 한 장의 이미지로 간단하지만 정확한 불량 검출", "이동중에. 길이 제한이 없는 고해상도 이미지로 정확한 불량 검출", "1MP~50MP Area 카메라", "2K ~ 8K Line 카메라"] },
      { key: "light", title: "조명", body: ["불량의 시인성에 따라 선택", "반사조명. 빛의 반사를 통하여 불량검출", "투과조명. 빛의 투과량을 통하여 불량검출", "그외. 제품에 맞는 맞춤 조명 솔루션"] },
      { key: "pc", title: "PC/소프트웨어", body: ["SW. 레시피 관리, 통계/리포트, 트레이스 로그", "PC. 검사속도에 반영하여", "일반검사. CPU i5, RAM 16Gb, SSD 500Gb, Window 10", "고속/딥러닝 검사. 일반검사에 GPU 추가, CPU i9"] },
      { key: "method", title: "검사방식", body: ["이미지 필터링(Filter, Blurring, Sharpening)", "서브픽셀 에지/코너", "ROI(Region of Interest) 설정 및 추출", "이진화(Binarization) ", "형태학적 연산(Morphological Operations)", "객체 추적(Object Tracking)", "영상 정합(Image Registration) "] },
      { key: "speed", title: "검사속도", body: ["원하는 속도에 맞춰", "최대 속도에 맞추어 카메라 선정 및 알고리즘 최적화"] },
  ...
  measur: {
    p1: { title: "비접촉 두께/변위 측정", bullets: ["공초점/레이저 변위 센서, 제품과 직접 접촉 없음", "제품 손상 최소화", "μm 단위 정밀 측정 솔루션"] },
    ...
    tabs: [
      ...
      { key: "motion", title: "구동방식", body: ["안정적인", "Belt Type 서보모터", "전면 커버 구성으로 안정성 확보", "석정반을 통한 정밀도 상승", "Needs에 맞는 맞춤 설계"] },
      { key: "pc", title: "PC/소프트웨어", body: ["SW. 레시피 관리, 통계/리포트, 트레이스 로그", "PC. 측정 속도에 반영하여", "표준 PC. CPU i5, RAM 16Gb, SSD 500Gb, Window 10"] },
      { key: "method", title: "각종기능", body: ["측정에 더한 편리한 기능", "검사 Data 저장(원하는 양식에 맞게 modify 가능) 및 Excel 출력 가능", "측정기 로그 저장으로 문제 발생시 원인 추적 가능", "설정 횟수만큼 반복 측정 가능", "Air Blow 설치하여 자동 클리닝 기능 추가 가능", "UI에 표기된 데이터 선택하여 저장 가능", "Calibration Module 탑재"] },
    ],
  },
};
            ...
            <div className="px-6 md:px-10 lg:px-14 pt-10 md:pt-12 text-center">
              <div className="translate-y-[16px] md:translate-y-[25px] space-y-2 md:space-y-3">
                ...
                {copy.p2.subs.map((line, i) => (
                  <p key={i} className="text-[16px] md:text-[20px] font-semibold text-black md:text-neutral-800 leading-relaxed">
                    {line}
                  </p>
                ))}
              </div>
              ...
              >
                <div className="px-6 py-5">
                  ...
                  {(() => {
                    ...
                    return <p className="text-[15px] leading-relaxed text-black md:text-neutral-800">{body}</p>;
                  })()}
                </div>
              </div>
            </div>
